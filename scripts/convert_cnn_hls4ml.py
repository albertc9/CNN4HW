...
def eval_numeric_dataset(hls_model, keras_model, npy_path, maxN=None, batch=256,
                         save_preds=False, save_path="preds_eval.npz", topk=50,
                         csv_out_dir=None, timestamp=None):
    p = Path(npy_path)
    if not p.exists():
    ...
    if not (X.ndim == 4 and X.shape[1:] == (4,256,1)):
        print(f"[WARN] Level-A skipped: sample is not a batch of shape (N,4,256,1). Got {X.shape}")
        return None
    ...
    # ---- 强制逐样本 ----
    yk_list, yh_list, diffs, idxs = [], [], [], []

    for i in range(N):
        xb = np.ascontiguousarray(X[i:i+1].astype(np.float32, copy=False))
        yk = keras_model.predict(xb, verbose=0).reshape(-1)
        yh = hls_model.predict(xb).reshape(-1)
        d  = np.abs(yk - yh)
        yk_list.append(yk)
        yh_list.append(yh)
        diffs.append(d)
    ...
    diffs  = np.concatenate(diffs, axis=0)  # shape (N,)

    mae  = float(diffs.mean())
    rmse = float(np.sqrt(np.mean(diffs**2)))
    maxd = float(diffs.max())
    print(f"\n[Level-A] NUMERIC DATASET (per-sample)  N={N}  MAE={mae:.6g}  RMSE={rmse:.6g}  MaxΔ={maxd:.6g}")

    # Sort ALL samples by descending error (largest error first)
    order = np.argsort(-diffs)  # Sort all by descending delta

    # Top-K outliers (console output)
    if topk and N > 0:
        top_order = order[:int(min(topk, len(diffs)))]
        print("[Level-A] Top-{} |Δ| samples (idx, y_keras, y_hls, Δ):".format(len(top_order)))
        for j in top_order:
            print(f"  #{idxs[j]:5d}: {yk_all[j]:.6f}  vs  {yh_all[j]:.6f}   Δ={diffs[j]:.6f}")

    # Save CSV with ALL samples sorted by error (largest first)
    if csv_out_dir:
        csv_dir = Path(csv_out_dir)
        csv_dir.mkdir(parents=True, exist_ok=True)
        ts = timestamp or datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_path = csv_dir / f"level_a_comparison_{ts}.csv"

        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['Rank', 'Sample_Index', 'y_keras', 'y_hls', 'abs_delta'])
        ...
        print(f"[Level-A] Saved CSV comparison (all {N} samples, sorted by |Δ|) to: {csv_path}")
...
def setup_logging(log_dir="out/log"):
    """Setup logging to both console and file with timestamp."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = Path(log_dir)
    log_path.mkdir(parents=True, exist_ok=True)
    log_file = log_path / f"convert_cnn_hls4ml_{timestamp}.log"
    ...
    return tee, timestamp


def main(C):
    # Setup logging with timestamp
    tee, timestamp = setup_logging(log_dir="out/log")
    ...
    try:
        ...
        eval_numeric_dataset(
            ...
            csv_out_dir="out/error_analysis",
            timestamp=timestamp
        )
        ...
        tee.close()
        print(f"\n[Info] Log saved to: out/log/convert_cnn_hls4ml_{timestamp}.log")
